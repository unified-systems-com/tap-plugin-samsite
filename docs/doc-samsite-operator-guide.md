# Samsite operator guide — standing it up yourself

> This is the full standup reference for running the `samsite` boot profile —
> credentials, region scoping, artifact manifests, collector ordering, and the
> failure modes that have actually bitten. It exists so the
> [README](../README.md) can stay focused on what Rampart does. You only need
> this document if you are standing up a samsite instance yourself; the
> reference deployment is already running and nobody else is expected to
> rebuild it.
